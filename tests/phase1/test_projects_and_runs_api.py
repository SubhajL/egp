...
def test_projects_endpoints_list_and_detail_repository_backed(tmp_path) -> None:
    ...
    assert listed.json()["projects"][0]["budget_amount"] == "1500000"
    assert detail.status_code == 200
    assert detail.json()["project"]["id"] == project.id
    assert detail.json()["project"]["budget_amount"] == "1500000"
    assert detail.json()["aliases"][0]["alias_type"] == "project_number"


def test_project_detail_endpoint_dedupes_status_timeline(tmp_path) -> None:
    database_url = f"sqlite+pysqlite:///{tmp_path / 'phase1.sqlite3'}"
    client = TestClient(
        create_app(
            artifact_root=tmp_path, database_url=database_url, auth_required=False
    ...
    with client.app.state.db_engine.begin() as connection:
        connection.execute(
            ...
            ),
            {
                ...
                "observed_status_text": "ประกาศเชิญชวน",
                "normalized_status": ProjectState.OPEN_INVITATION.value,
...
def test_run_log_endpoint_reads_legacy_absolute_log_path_via_artifact_root(
    tmp_path,
) -> None:
    database_url = f"sqlite+pysqlite:///{tmp_path / 'phase1.sqlite3'}"
    ...
    log_path = artifact_root / "tenants" / TENANT_ID / "runs" / run_id / "worker.log"
    log_path.parent.mkdir(parents=True, exist_ok=True)
    log_path.write_text("legacy absolute log\n", encoding="utf-8")
    ...
    run = repository.create_run(
        ...
        run_id=run_id,
        summary_json={
            "worker_log_path": "/Users/subhajlimanond/dev/egp/.data/artifacts/tenants/"
            f"{TENANT_ID}/runs/{run_id}/worker.log"
        },
    )
    ...
    assert response.status_code == 200
    assert response.text == "legacy absolute log\n"
...
def test_project_ingest_discover_endpoint_rejects_late_stage_first_discovery(
    ...
    database_url = (
        f"sqlite+pysqlite:///{tmp_path / 'phase1-project-ingest-late.sqlite3'}"
    )
    client = TestClient(
        create_app(
            artifact_root=tmp_path,
