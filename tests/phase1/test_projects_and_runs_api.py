...
def test_projects_endpoints_list_and_detail_repository_backed(tmp_path) -> None:
    ...
    assert listed.json()["projects"][0]["canonical_project_id"] == "project-number:EGP-2026-1001"
...
def test_runs_endpoints_create_list_and_return_tasks(tmp_path) -> None:
    ...
    created = client.post("/v1/runs", json={"tenant_id": TENANT_ID, "trigger_type": "manual"})
    ...
    client.post(
        ...
        params={"tenant_id": TENANT_ID},
        json={"status": CrawlRunStatus.SUCCEEDED.value, "summary_json": {"projects_seen": 1}},
    )
...
def test_projects_and_runs_endpoints_accept_limit_and_offset(tmp_path) -> None:
    ...
    first_run = client.post("/v1/runs", json={"tenant_id": TENANT_ID, "trigger_type": "manual"}).json()
    second_run = client.post("/v1/runs", json={"tenant_id": TENANT_ID, "trigger_type": "retry"}).json()

    projects_page = client.get("/v1/projects", params={"tenant_id": TENANT_ID, "limit": 1, "offset": 1})
    runs_page = client.get("/v1/runs", params={"tenant_id": TENANT_ID, "limit": 1, "offset": 1})
