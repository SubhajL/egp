...
def test_get_browser_profile_mode_defaults_per_run(
...
def test_get_browser_profile_mode_accepts_persistent(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_get_browser_profile_mode_rejects_invalid(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_get_browser_chrome_path_none_by_default(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_get_browser_proxy_server_none_by_default(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_get_browser_proxy_server_rejects_credentials(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_get_browser_use_xvfb_truthy(
...
def test_get_browser_nav_timeout_rejects_nonpositive(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_get_browser_cloudflare_timeout_defaults(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_get_browser_project_detail_timeout_defaults(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_get_browser_project_detail_timeout_from_env(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_get_browser_warmup_failure_pause_threshold_defaults_and_allows_zero(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.delenv("EGP_BROWSER_WARMUP_FAILURE_PAUSE_THRESHOLD", raising=False)
    assert get_browser_warmup_failure_pause_threshold() == 2
    monkeypatch.setenv("EGP_BROWSER_WARMUP_FAILURE_PAUSE_THRESHOLD", "0")
    assert get_browser_warmup_failure_pause_threshold() == 0


def test_get_browser_warmup_failure_pause_threshold_rejects_negative(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("EGP_BROWSER_WARMUP_FAILURE_PAUSE_THRESHOLD", "-1")
    with pytest.raises(RuntimeError):
        get_browser_warmup_failure_pause_threshold()
